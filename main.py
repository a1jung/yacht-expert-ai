...
async def ask(request: Request):
    try:
        ...
        if matches:
            ...
            # 추천 질문: 다른 질문 존재 시 샘플링
            other_questions = [q["질문"] for q in sailing_data
                               if isinstance(q, dict) and "질문" in q
                               and q["질문"] != best_match["질문"]]
            recommended = random.sample(other_questions, min(3, len(other_questions))) if other_questions else []
        else:
            ...
            all_questions = [q["질문"] for q in sailing_data if isinstance(q, dict) and "질문" in q]
            recommended = random.sample(all_questions, min(3, len(all_questions))) if all_questions else []
