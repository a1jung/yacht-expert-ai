...
async def ask(request: Request):
    try:
        ...
        if matches:
            ...
            # 추천 질문 생성 (데이터에 있는 다른 질문 중 3개 선택)
            other_questions = [q["질문"] for q in sailing_data if q["질문"] != best_match["질문"]]
            ...
            recommended = random.sample([q["질문"] for q in sailing_data], min(3, len(sailing_data)))
